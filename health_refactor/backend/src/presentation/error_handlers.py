...
from backend.src.presentation.schemas.api_response import error_body
...
def register_error_handlers(app: FastAPI) -> None:
    ...
    async def handle_oauth_not_configured(
        _request: Request, exc: OAuthNotConfiguredError
    ) -> JSONResponse:
        return _json_error(
            message=str(exc) or "Google OAuth is not configured",
            status_code=503,
        )
