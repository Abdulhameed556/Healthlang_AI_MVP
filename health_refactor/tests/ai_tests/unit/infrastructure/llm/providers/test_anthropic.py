"""Unit tests: ai/src/infrastructure/llm/providers/anthropic.py"""
...
@pytest.fixture()
def provider() -> AnthropicSingleTaskAgentProvider:
    return AnthropicSingleTaskAgentProvider(api_key="test-key")


@pytest.mark.asyncio
async def test_run_returns_content_and_usage(provider: AnthropicSingleTaskAgentProvider) -> None:
    ...
    with patch(
        "ai.src.infrastructure.llm.providers.anthropic.ChatAnthropic",
...
@pytest.mark.asyncio
async def test_run_flattens_content_blocks(provider: AnthropicSingleTaskAgentProvider) -> None:
    mock_message = MagicMock()
    mock_message.content = [{"type": "text", "text": "Hello from blocks!"}]
    mock_message.usage_metadata = None
    mock_llm = MagicMock()
    mock_llm.ainvoke = AsyncMock(return_value=mock_message)

    with patch("ai.src.infrastructure.llm.providers.anthropic.ChatAnthropic", return_value=mock_llm):
        result = await provider.run(_request())

    assert result.content == "Hello from blocks!"


@pytest.mark.asyncio
async def test_stream_yields_chunks(provider: AnthropicSingleTaskAgentProvider) -> None:
    chunk_a = MagicMock()
    chunk_a.content = "Hel"
    chunk_b = MagicMock()
    chunk_b.content = "lo"
...
@pytest.mark.asyncio
async def test_run_includes_message_history(provider: AnthropicSingleTaskAgentProvider) -> None:
    mock_message = MagicMock()
...
@pytest.mark.asyncio
async def test_run_structured_parses_json(provider: AnthropicSingleTaskAgentProvider) -> None:
    mock_message = MagicMock()
...
def test_requires_api_key() -> None:
    with pytest.raises(ValueError, match="ANTHROPIC_API_KEY"):
        AnthropicSingleTaskAgentProvider(api_key="")
