"""Unit tests: ai/src/infrastructure/llm/providers/groq.py"""
...
@pytest.fixture()
def provider() -> GroqSingleTaskAgentProvider:
    return GroqSingleTaskAgentProvider(api_key="test-key")


@pytest.mark.asyncio
async def test_run_returns_content_and_usage(provider: GroqSingleTaskAgentProvider) -> None:
    ...
    with patch(
        "ai.src.infrastructure.llm.providers.groq.ChatGroq",
...
@pytest.mark.asyncio
async def test_run_reads_usage_from_response_metadata(provider: GroqSingleTaskAgentProvider) -> None:
    mock_message = MagicMock()
    mock_message.content = "Hello!"
    ...
    with patch("ai.src.infrastructure.llm.providers.groq.ChatGroq", return_value=mock_llm):
        result = await provider.run(_request())
...
@pytest.mark.asyncio
async def test_stream_yields_chunks(provider: GroqSingleTaskAgentProvider) -> None:
    chunk_a = MagicMock()
    chunk_a.content = "Hel"
    chunk_b = MagicMock()
    chunk_b.content = "lo"
...
@pytest.mark.asyncio
async def test_run_includes_message_history(provider: GroqSingleTaskAgentProvider) -> None:
    mock_message = MagicMock()
...
@pytest.mark.asyncio
async def test_run_structured_parses_json(provider: GroqSingleTaskAgentProvider) -> None:
    mock_message = MagicMock()
...
def test_requires_api_key() -> None:
    with pytest.raises(ValueError, match="GROQ_API_KEY"):
        GroqSingleTaskAgentProvider(api_key="")
